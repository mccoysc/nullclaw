...
| 变更 | 效果 |
|------|------|
| 全局模型参数（`provider`、`model`、`max_context_tokens`、`temperature`） | 无 per-endpoint 覆盖的会话原地热更新，对话历史保留。 |
| 全局子 agent / 工具审查器模型（`sub_agent_provider`、`sub_agent_model`、`tools_reviewer_provider`、`tools_reviewer_model`） | 同全局模型参数——无 per-endpoint 覆盖的会话热更新。 |
